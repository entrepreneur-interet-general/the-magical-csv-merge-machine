...
# -*- coding: utf-8 -*-
"""
Created on Thu Oct  5 12:36:09 2017
...
def _gen_bulk(index_name, search_templates, must, must_not, num_results, chunk_size=100):
    '''
    Create a generator that yields bulk requests that cover all search templates
...
def _bulk_search(es, index_name, all_query_templates, rows, must_filters, must_not_filters, num_results=3):
    ...
    NB: originaly perform_request
    
    INPUT:
        - es: instance of Elasticseach class (from elasticsearch library)
        - index_name: the index to search from
        - all_query_templates: iterator of queries to perform
        - rows: iterator of pandas.Series containing the rows to match
        - must_filters: {column_name: ['strings', 'to', 'include']}-
        - must_not_filters: {column_name: ['strings','to exclude']}
    ...
    # og_search_template is [(0, (query, row)), ...]; full_responses is {0: res, 1: res, ...}
