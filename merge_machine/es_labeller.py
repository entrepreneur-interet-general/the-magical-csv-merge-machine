...
class SingleQueryTemplate():
    ...
    def to_dict(self):
        '''Returns a dict representation of'''
...
class CompoundQueryTemplate():
    ...
    def to_dict(self):
        ...
        return {'musts': [x.to_dict() for x in self.musts],
                'shoulds': [x.to_dict() for x in self.shoulds]}
    ...
    def from_dict(cls, dict_):
        ...
        single_query_templates_tuple = [SingleQueryTemplate.from_dict(d)._as_tuple() \
                                        for d in dict_['musts'] + dict_['shoulds']]
...
class LabellerQuery(CompoundQueryTemplate):
    ...
    def to_dict(self):
        ...
        dict_['has_results'] = self.has_results
    ...
    def from_dict(cls, dict_):
        ...
        Returns an instance of the class using a representation generated by to_dict
        '''
        labeller = super(LabellerQuery, cls).from_dict(cls, dict_)
        labeller['history_pairs'] = dict_['history_pairs']
        labeller['first_scores'] = dict_['first_scores']
        labeller['has_results'] = dict_['has_results']
        labeller['first_scores'] = dict_['first_scores']
    ...
    def add_pairs(self, pairs, first_score):
        ...
        self.has_results.append(bool(pairs))
    ...
    def previous(self, num_labelled):
        ...
        self.has_results = self.has_results[:idx+1]
    ...
    def compute_metrics(self, t_p=0.95, t_r=0.3):
        ...
        OUTPUT:
            ...
            - score
        '''
        summaries = [{'score': s, 'first_is_match': fim, 'any_is_match': aim, 'has_results': hr} \
                for (s, fim, aim, hr) in zip(self.first_scores, self.first_is_match, 
                                        self.any_is_match, self.has_results)]
...
class Labeller():
    ...
    # TODO: track last expansion not to over expand
...
#        Returns an instance of the class using a representation generated by to_dict
#        '''
#        labeller = super().from_dict(dict_)
#        labeller['history_pairs'] = dict_['history_pairs']
#        labeller['first_scores'] = dict_['first_scores']
#        labeller['has_results'] = dict_['has_results']
#        labeller['first_scores'] = dict_['first_scores']
...
#        self.first_scores.append(first_score)
    ...
    def _re_score_history(self, call_next_row):        
        '''
        Performes queries for current_queries, re-scores and sorts. Use this
        after adding self.musts_filters and/or self.must_not_filters or after
        expansion of current_queries
    ...
    def expand(self):
        EXPAND_FREQ = 4 # TODO: smarter than that

        try:
            self.last_expanded += 1
        except:
            self.last_expanded = 1000

        if self.last_expanded <= EXPAND_FREQ: # TODO: all this is ugly just to have 1 expansion
            return

        if self._nprl() in [7, 17]:
            print('EXPANDING !')
            self.expand_by_boost()
            
            self.last_expanded = 0
        elif (self._nprl() > 7) & (self._nprl() % EXPAND_FREQ == 0) :
            self.expand_by_core()
        ...
        #                self.expand_by_core()
        #        
        #            self._re_score_history(next_row=False)
        #            self.filter_()
    ...
    def expand_by_boost(self):
        ...
        l2 = len(self.current_queries)
        print('expand_by_boost: added {0} queries; {1} left'.format(l2-l1, l2))
    ...
    def to_emit(self):
        ...
        #dict_to_emit['source_item'] = self.current_source_item.to_dict()
        #dict_to_emit['ref_item'] = self.current_ref_item # TODO: change front to take this into account
        
        dict_to_emit['source_item'] = {'_id': self.current_source_idx, 
                                        '_source': self.current_source_item.to_dict()}
