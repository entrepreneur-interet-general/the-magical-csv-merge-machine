...
# Standard modules
import csv, itertools, re, unicodedata, logging, optparse, sys
...
class SubtypeMatcher(TypeMatcher):
    def __init__(self, t, subtypes):
        ...
        if len(subtypes) < 1:
            raise Exception('Invalid subtype matcher setup')
...
class CustomDateMatcher(TypeMatcher):
    ...
    def match(self, c):
        ...
        try:
            ...
            logging.error('Error while parsing date %s: %s', c.value, e)

def scorePhoneNumber(z): 
    return 100 if phonenumbers.is_valid_number(z) else 75 if phonenumbers.is_possible_number(z) else 5

def normalizePhoneNumber(z): 
